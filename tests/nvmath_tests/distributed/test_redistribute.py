...
from nvmath.distributed.distribution import BlockNonCyclic, Box, Slab
...
def test_unsupported_itemsize(dtype, nvmath_distributed, check_symmetric_memory_leaks):
    ...
    with pytest.raises(
        ...
        nvmath.distributed.distribution.redistribute(data, input_distribution=box, output_distribution=box)
...
def test_wrong_boxes1(nvmath_distributed, check_symmetric_memory_leaks):
    ...
    with pytest.raises(
        ...
        nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((0, 0), (2, 2)), output_distribution=Box((0, 0), (2, 2))
        )
...
def test_wrong_boxes2(nvmath_distributed, check_symmetric_memory_leaks):
    ...
    with pytest.raises(
        ...
        if rank % 2 == 0:
            ...
            nvmath.distributed.distribution.redistribute(
                data, input_distribution=Box((4, 4), (6, 6)), output_distribution=Box((4, 4), (6, 6))
            )
...
def test_wrong_boxes3(nvmath_distributed, check_symmetric_memory_leaks):
    ...
    with pytest.raises(
        ...
        data = np.array([0, 1, 2, 3], dtype=np.int32).reshape((2, 2))
        nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((2, 2), (0, 0)), output_distribution=Box((2, 2), (0, 0))
        )
...
def test_redistribute_matrix_2_processes(memory_order, dtype, nvmath_distributed, check_symmetric_memory_leaks):
    distributed_ctx = nvmath.distributed.get_context()
    process_group = distributed_ctx.process_group
    rank = process_group.rank
    nranks = process_group.nranks
    ...
    # Redistribute matrix of global shape (2, 3) in this fashion:
    #
    # |0  1  |2 |           |0 |1  2 |
    # |3  4  |5 |     =>    |3 |4  5 |
    #
    ...
    if rank == 0:
        ...
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((0, 0), (2, 2)), output_distribution=Box((0, 0), (2, 1))
        )
        expected = np.array([0, 3], dtype=dtype).reshape((2, 1))
    else:
        data = F(np.array([2, 5], dtype=dtype).reshape((2, 1)))
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((0, 2), (2, 3)), output_distribution=Box((0, 1), (2, 3))
        )
...
def test_redistribute_matrix_4_processes(memory_order, dtype, nvmath_distributed, check_symmetric_memory_leaks):
    distributed_ctx = nvmath.distributed.get_context()
    process_group = distributed_ctx.process_group
    rank = process_group.rank
    nranks = process_group.nranks
    ...
    # Redistribute matrix of global shape (4, 4) in this fashion:
    #
    # |0  1  |2  3 |          |0 |1 |2 |3 |
    # |4  5  |6  7 |          |4 |5 |6 |7 |
    # --------------    =>    |8 |9 |10|11|
    ...
    if rank == 0:
        ...
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((0, 0), (2, 2)), output_distribution=Box((0, 0), (4, 1))
        )
        expected = np.array([0, 4, 8, 12], dtype=dtype).reshape((4, 1))
    elif rank == 1:
        data = F(np.array([(2, 3), (6, 7)], dtype=dtype))
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((0, 2), (2, 4)), output_distribution=Box((0, 1), (4, 2))
        )
        expected = np.array([1, 5, 9, 13], dtype=dtype).reshape((4, 1))
    elif rank == 2:
        data = F(np.array([(8, 9), (12, 13)], dtype=dtype))
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((2, 0), (4, 2)), output_distribution=Box((0, 2), (4, 3))
        )
        expected = np.array([2, 6, 10, 14], dtype=dtype).reshape((4, 1))
    else:
        data = F(np.array([(10, 11), (14, 15)], dtype=dtype))
        result = nvmath.distributed.distribution.redistribute(
            data, input_distribution=Box((2, 2), (4, 4)), output_distribution=Box((0, 3), (4, 4))
        )
...
@pytest.mark.parametrize("use_unchecked_reset", [False, True])
def test_execute_raises_after_release_operand(
    input_memory_space, use_unchecked_reset, nvmath_distributed, check_symmetric_memory_leaks
):
    distributed_ctx = nvmath.distributed.get_context()
    ...
    with nvmath.distributed.distribution.Redistribute(data_in.tensor, box, box) as redistribute:
        redistribute.plan()
        result1 = redistribute.execute()
        result2 = redistribute.execute()
        redistribute.release_operand()
        ...
        if use_unchecked_reset:
            redistribute.reset_operand_unchecked(data_in.tensor)
        else:
            redistribute.reset_operand(data_in.tensor)
        result3 = redistribute.execute()
...
def test_release_operand_refcount(input_memory_space, release_after, nvmath_distributed, check_symmetric_memory_leaks):
    ...
    redistribute = nvmath.distributed.distribution.Redistribute(a, box, box)
...
def test_redistribute(
    ...
    ranks according to the X-slab distribution used by cuFFTMp, and redistributes it
    to Y-slab distribution using the Redistribute operation. To verify correctness,
    ...
    if use_unchecked_reset and reset_inplace:
        # reset_inplace skips the reset_operand call, so use_unchecked_reset
        # has no effect.
        return
    ...
    # Redistribute from X-slab to Y-slab distribution.
    ...
    with nvmath.distributed.distribution.Redistribute(data_in.tensor, input_box, output_box, options=options) as redistribute:
        redistribute.plan()
        # We'll do two redistributes per test case (to test reset_operand).
        ...
        while True:
            ...
            original_data_in = data_in.__class__.empty(
                ...
                symmetric_memory="nvshmem" if input_memory_space == "gpu" else None,
            )
            ...
            if redistribute_count < 2:
                ...
                if input_memory_space == "gpu" and reset_inplace:
                    ...
                    #         symmetric_memory=(input_memory_space == "gpu"),
                    #     )
                    #     redistribute.reset_operand(data_in.tensor, out=out.tensor)
                    # else:
                    #     # assert redistribute.out is None
                    if use_unchecked_reset:
                        redistribute.reset_operand_unchecked(data_in.tensor)
                    else:
                        redistribute.reset_operand(data_in.tensor)
...
def test_redistribute_1D(package, nvmath_distributed, check_symmetric_memory_leaks):
    """This test redistributes a 1D array that is evenly partitioned across ranks
    ...
    # Run redistribute.
    result = nvmath.distributed.distribution.redistribute(data_in.tensor, input_box, output_box)
    result = dist_wrap_operand(result)
    ...
    if rank == 0:
        result_global = result.__class__.empty(global_shape, result.device_id, dtype=result.dtype, symmetric_memory=None)
...
@pytest.mark.parametrize("global_shape", [(64, 64), (128, 256), (512, 64), (63, 48), (32, 61), (31, 31)])
@pytest.mark.parametrize("from_to", [(Slab.X, Slab.Y), (Slab.Y, Slab.X), (Slab.X, Slab.X), (Slab.Y, Slab.Y)])
def test_redistribute_slab(global_shape, from_to, nvmath_distributed, check_symmetric_memory_leaks):
    distributed_ctx = nvmath.distributed.get_context()
    process_group = distributed_ctx.process_group
    rank = process_group.rank

    input_distribution, output_distribution = from_to

    # x is distributed according to input_distribution
    x = np.random.rand(*input_distribution.shape(rank, global_shape))

    # re-distribute to output_distribution
    y = nvmath.distributed.distribution.redistribute(x, input_distribution, output_distribution)

    assert y is not x

    # Gather x on its partition axis.
    x_global = gather_array(dist_wrap_operand(x), input_distribution.partition_dim, process_group, rank)
    # Gather y on its partition axis.
    y_global = gather_array(dist_wrap_operand(y), output_distribution.partition_dim, process_group, rank)

    if rank == 0:
        try:
            assert x_global.shape == y_global.shape == global_shape
            assert_close(x_global, y_global, rtol=0)
            process_group_broadcast(process_group, None)
        except Exception as e:
            # Broadcast the exception to avoid deadlock.
            process_group_broadcast(process_group, e)
...
@pytest.mark.parametrize("global_shape", [(64, 64), (128, 256), (512, 64)])
@pytest.mark.parametrize(
    "from_to",
    [(Slab, Box), (Box, Slab), (Slab, BlockNonCyclic), (BlockNonCyclic, Box), (Box, BlockNonCyclic), (BlockNonCyclic, Slab)],
)
def test_redistribute_mixed_distribution_types(global_shape, from_to, nvmath_distributed, check_symmetric_memory_leaks):
    """Test that redistribute works with mixed distribution types"""
    distributed_ctx = nvmath.distributed.get_context()
    process_group = distributed_ctx.process_group
    rank = process_group.rank

    input_distribution_type, output_distribution_type = from_to

    # NOTE: Currently this test only exercises 1D distribution, even with BlockNonCyclic.

    if input_distribution_type is Slab:
        input_distribution = Slab.X
    elif input_distribution_type is BlockNonCyclic:
        input_distribution = Slab.X.to(BlockNonCyclic, ndim=len(global_shape))
        assert type(input_distribution) is BlockNonCyclic
    elif input_distribution_type is Box:
        input_distribution = Slab.X.copy()._bind(global_shape).to(Box)
        assert type(input_distribution) is Box
    input_partition_dim = 0

    if output_distribution_type is Slab:
        output_distribution = Slab.Y
    elif output_distribution_type is BlockNonCyclic:
        output_distribution = Slab.Y.to(BlockNonCyclic, ndim=len(global_shape))
        assert type(output_distribution) is BlockNonCyclic
    elif output_distribution_type is Box:
        output_distribution = Slab.Y.copy()._bind(global_shape).to(Box)
        assert type(output_distribution) is Box
    output_partition_dim = 1

    # Hack since there are no public APIs that return bound distributions, and so
    # currently distributed ops expect unbound distributions.
    input_distribution._bound = False
    output_distribution._bound = False
    ...
    # Gather x on its partition axis.
    x_global = gather_array(dist_wrap_operand(x), input_partition_dim, process_group, rank)
    # Gather y on its partition axis.
    y_global = gather_array(dist_wrap_operand(y), output_partition_dim, process_group, rank)
...
def test_reset_operand_none(with_release, nvmath_distributed, check_symmetric_memory_leaks):
    ...
    stream = get_or_create_stream(device_id, stream=None, op_package="cupy")
    _, data_in = generate_random_data(np, "gpu", shape, dtype, stream)

...
from nvmath.sparse.ust import TensorFormat
...
JIT_AVAILABLE = numba is not None and nc is not None
...
cupyx_supported_value_dtypes = [DType.float32, DType.float64, DType.complex64, DType.complex128]

# as opposed to framework2dtype,
# where there are dtypes that are fine for dense matrices,
# this is a subset of dtypes that given framework supports as values
# of sparse matrices
framework2value_dtype = {
    Framework.torch: torch_dtype,
    Framework.cupyx: cupyx_supported_value_dtypes,
    Framework.scipy: scipy_dtype,
}

framework2sparse_array_type_support = {
    Framework.torch: [SparseArrayType.COO, SparseArrayType.CSR, SparseArrayType.CSC, SparseArrayType.BSR, SparseArrayType.BSC],
    # TODO(jlisowski): todia() is not implemented for cupy: SparseArrayType.DIA
    Framework.cupyx: [SparseArrayType.COO, SparseArrayType.CSR, SparseArrayType.CSC],
    Framework.scipy: [SparseArrayType.COO, SparseArrayType.CSR, SparseArrayType.CSC, SparseArrayType.BSR, SparseArrayType.DIA],
}

sparse_supporting_frameworks = [Framework.cupyx, Framework.scipy, Framework.torch]


def sparse_array_type_from_array(a):
    framework = framework_from_array(a)
    match framework:
        case Framework.torch:
            match a.layout:
                case torch.sparse_csr:
                    return SparseArrayType.CSR
                case torch.sparse_csc:
                    return SparseArrayType.CSC
                case torch.sparse_coo:
                    return SparseArrayType.COO
                case torch.sparse_bsr:
                    return SparseArrayType.BSR
                case torch.sparse_bsc:
                    return SparseArrayType.BSC
                case _:
                    raise ValueError(f"Unsupported sparse array type {a.layout}")

        case Framework.cupyx:
            if isinstance(a, cp.sparse.coo_matrix):
                return SparseArrayType.COO
            elif isinstance(a, cp.sparse.csr_matrix):
                return SparseArrayType.CSR
            elif isinstance(a, cp.sparse.csc_matrix):
                return SparseArrayType.CSC
            elif isinstance(a, cp.sparse.dia_matrix):
                return SparseArrayType.DIA
            else:
                raise ValueError(f"Unsupported sparse array type {type(a)}")
        case Framework.scipy:
            if isinstance(a, sp.coo_matrix):
                ...
                return SparseArrayType.CSC
            elif isinstance(a, sp.bsr_matrix):
                return SparseArrayType.BSR
            elif isinstance(a, sp.dia_matrix):
                return SparseArrayType.DIA
            else:
                raise ValueError(f"Unsupported sparse array type {type(a)}")
        case _:
...
def value_data_from_array(a):
    framework = framework_from_array(a)
    match framework:
        case Framework.torch:
            return a.values()
        case Framework.cupy | Framework.cupyx | Framework.scipy:
            return a.data
        case _:
            raise ValueError(f"Unsupported framework {framework}")
...
def copy_array(a):
    ...
    match framework:
        case Framework.torch:
            return a.clone()
        case Framework.scipy | Framework.numpy:
            return a.copy()
...
class Param:
    ...
    def pretty_name(self):
        if isinstance(self.value, (Enum, TensorFormat)):

...
def main():
    ...
    @cuda.jit
    def f(input, output):
        thread_data = cuda.local.array(shape=(storage_size,), dtype=value_type, alignment=32)
        shared_mem = cuda.shared.array(shape=(0,), dtype=value_type, alignment=32)
    ...
    try:
        ...
        if str(e) == "libcudadevrt.a not found":
            print(
                f"\n=== cuSIMT linker error: {e}. Please use the System CTK option (see Installation in the documentation) "
                "to run this example. ===\n"
            )
        raise e
    cuda.synchronize()

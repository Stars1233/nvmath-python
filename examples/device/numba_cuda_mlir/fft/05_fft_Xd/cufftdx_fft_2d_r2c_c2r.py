...
#
# Mirrors https://github.com/NVIDIA/CUDALibrarySamples/blob/main/MathDx/cuFFTDx/05_fft_Xd/fft_2d_r2c_c2r.cu
#
...
def main():
    ...
    def f_y_r2c(input, output):
        thread_data = cuda.local.array(shape=(storage_size_r2c,), dtype=complex_type, alignment=32)
        thread_data_real = thread_data.view(real_type)
        shared_mem = cuda.shared.array(shape=(0,), dtype=complex_type, alignment=32)
    ...
    def f_x(input, output):
        thread_data = cuda.local.array(shape=(storage_size_c2c,), dtype=complex_type, alignment=32)
        shared_mem = cuda.shared.array(shape=(0,), dtype=complex_type, alignment=32)
        ...
        # for i in range(ept_x):
        #     if index < fft_size_x:
        #         thread_data[i] = np.complex64(thread_data[i]) / (fft_size_x * fft_size_y)
        #         index += stride_c2c
    ...
    def f_y_c2r(input, output):
        thread_data = cuda.local.array(shape=(storage_size_c2r,), dtype=complex_type, alignment=32)
        thread_data_real = thread_data.view(real_type)
        shared_mem = cuda.shared.array(shape=(0,), dtype=complex_type, alignment=32)
    ...
    real = np.random.uniform(-1, 1, size=(fft_size_x, fft_size_y)).astype(np.float32)

...
def main():
    ...
    def f(inout):
        # Registers
        complex_thread_data = cuda.local.array(shape=(fft.storage_size,), dtype=fft.value_type, alignment=32)
        ...
        for i in range(fft.elements_per_thread):
            if i * fft.stride + cuda.threadIdx.x < fft.size // 2:
                complex_thread_data[i] = np.complex64(complex_thread_data[i]) / fft.size

...
#
# Mirrors https://github.com/NVIDIA/CUDALibrarySamples/blob/main/MathDx/cuSolverDx/02_Least_Squares/gels_batched_block.cu
#
...
def reconstruct_qr(a: np.ndarray, tau: np.ndarray, n_rows: int, tau_size: int) -> tuple[np.ndarray, np.ndarray]:
    r = np.triu(a)
...
def reconstruct_lq(a: np.ndarray, tau: np.ndarray, n_cols: int, tau_size: int) -> tuple[np.ndarray, np.ndarray]:
    lower = np.tril(a)

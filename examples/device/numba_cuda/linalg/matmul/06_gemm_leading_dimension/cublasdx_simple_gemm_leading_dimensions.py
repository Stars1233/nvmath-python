...
#
# Mirrors https://github.com/NVIDIA/CUDALibrarySamples/blob/main/MathDx/cuBLASDx/06_gemm_leading_dimension/simple_gemm_leading_dimensions.cu
#
...
def main():
    ...
    def f_static_ld(alpha, a, b, beta, c, output):
        ...
        load_to_shared(a, smem_a, a_dim, lda, row_major=True)
        load_to_shared(b, smem_b, b_dim, ldb)
        load_to_shared(c, smem_c, c_dim, ldc)

        cuda.syncthreads()

        MM_static_ld.execute(alpha, smem_a, smem_b, beta, smem_c)
    ...
    def f_runtime_ld(alpha, a, lda, b, ldb, beta, c, ldc, output):
        ...
        MM_runtime_ld.execute(alpha, smem_a, lda, smem_b, ldb, beta, smem_c, ldc)
    ...
    shared_memory_size = MM_static_ld.get_shared_storage_size()
    f_static_ld[1, block_dim, 0, shared_memory_size](alpha, a_d, b_d, beta, c_d, o_static_ld_d)
    cuda.synchronize()

    shared_memory_size = MM_runtime_ld.get_shared_storage_size(lda, ldb, ldc)

...
* **RHS only changes** — keep the existing factorization of ``a``; call ``factorize()`` only
  when ``a`` changes.
* **LHS changes** — call ``factorize()`` again before the next ``solve()``.

Operands are real CuPy arrays on the GPU. See ``example04_reset_operands_numpy.py`` for
the same flow with NumPy arrays on the CPU.
"""
...
with la.DirectSolver(a0, b0) as solver:
    ...
    # 1) Solve a0 @ x0 = b0.
    x0 = solver.solve()
    cp.testing.assert_allclose(a0 @ x0, b0, rtol=rtol, atol=atol)

    # 2) Same matrix a0, new rhs b1 — refresh rhs only; reuse the existing factorization.
    solver.reset_operands(b=b1)
    y = solver.solve()
    cp.testing.assert_allclose(a0 @ y, b1, rtol=rtol, atol=atol)
    ...
    solver.factorize()
    z = solver.solve()
    cp.testing.assert_allclose(a1 @ z, b1, rtol=rtol, atol=atol)

# Default stream may still hold work; synchronize before using results on the host.

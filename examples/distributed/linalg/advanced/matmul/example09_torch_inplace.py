...
$ mpiexec -n 4 python example09_torch_inplace.py
"""
...
# Prepare sample input data (PyTorch tensors, on the GPU).
...
# cuBLASMp requires Fortran memory layout. PyTorch allocates C-ordered arrays by default,
...
with torch.cuda.device(device_id):
    a = torch.rand(*a_shape, device="cuda").T
    b = torch.rand(*b_shape, device="cuda").T
    c = torch.rand(*c_shape, device="cuda").T
...
if rank == 0:
    ...
    print(result.shape, result.stride())

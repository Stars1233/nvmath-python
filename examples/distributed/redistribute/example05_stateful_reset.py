...
"""
This example illustrates how to reuse the stateful API to redistribute operands
with the same input distribution.

$ mpiexec -n 4 python example05_stateful_reset.py
"""
...
with nvmath.distributed.distribution.Redistribute(a, Slab.Y, Slab.X) as r:
    ...
    # Reset the operand. Note that the new operand must have the same distribution,
    # based on the same global shape (partitioned on Y axis).
    c = nvmath.distributed.allocate_symmetric_memory(shape, cp, dtype=cp.complex64)
    with cp.cuda.Device(device_id):
        c[:] = cp.random.rand(*shape, dtype=cp.float32) + 1j * cp.random.rand(*shape, dtype=cp.float32)
    r.reset_operand(c)

...
# the X axis according to Slab.X distribution, with redistribute=False so that the forward
...
# Before redistribute executes, the local changes to the input operand must be visible
# on all GPUs. This is achieved by issuing a NVSHMEM sync_all operation on the stream
# on which the operand is prepared. nvmath-python's Redistribute by default
# issues a synchronization on the execute stream. The same behavior is achieved
# with sync_symmetric_memory=True.
b = nvmath.distributed.distribution.redistribute(a, Slab.Y, Slab.X, sync_symmetric_memory=True)

# Now that we have operand b with Slab.X, we can run the forward FFT with Slab.X
# distribution and redistribute=False to get the result in Slab.Y distribution.
...
with nvmath.distributed.fft.FFT(b, distribution=Slab.X, options={"redistribute": False}) as f:
    # Plan the FFT.
    f.plan()

    # After the library redistributes the output, it issues a symmetric memory
    # synchronization on the execute stream. As such, there is no need to issue
    # another one here (we disable it with sync_symmetric_memory=False).

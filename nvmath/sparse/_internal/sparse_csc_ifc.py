...
"""
A generic type for the third-party CSCTensor which a CSCTensorHolder implementation wraps
around.
"""
CSCTensor = TypeVar("CSCTensor")


class CSCTensorHolder(SparseTensorHolder):
    """
    A simple wrapper type for CSC sparse tensors to make the API package-agnostic. The
    ...
    _format_name: str = "CSC"

    def __init__(self, ccol_indices, row_indices, values, *, attr_name_map=None, shape=None, tensor=None):
    ...
    def create_from_tensor(cls, tensor, *, attr_name_map=None):
        ...
        return CSCTensorHolder(ccol_indices, row_indices, values, attr_name_map=attr_name_map, tensor=tensor)
    ...
    def format_name(self) -> str:
        return CSCTensorHolder._format_name
    ...
    def to(self, device_id: int | Literal["cpu"], stream_holder: StreamHolder | None):
        """Copy the CSCTensor representation to a different device.
        ...
        return CSCTensorHolder(
            target_ccol_indices, target_row_indices, target_values, attr_name_map=self.attr_name_map, shape=self.shape
        )

    def copy_(self, src, stream_holder: StreamHolder | None) -> None:
        """Overwrite self.tensor (in-place) with a copy of src (CSCTensor)."""
    ...
    def to_ust(self, *, stream):
        ...
        if not (wrapped.attr_name_map["is_coalesced"](tensor) and wrapped.attr_name_map["has_sorted_indices"](tensor)):
            raise ValueError(
                "The CSC tensor is not coalesced (metadata sorted with duplicates removed). \
...
dense and contiguous."
        ...
        tensor_format = NamedFormats.CSCd(num_batch_dim, num_dense_dim)
...
and values() tensors) must use the C-layout."
    ...
    def to_package(self):
        ...
        """
        create_csc = self.attr_name_map["sparse_format_helper"].create_csc
        return create_csc(self.shape, self.ccol_indices, self.row_indices, self.values)

...
class cuFFTMpError(Exception):
    ...
    def __init__(self, status):
        ...
        super(cuFFTMpError, self).__init__(err)
...
cpdef xt_set_distribution(int plan, int rank, lower_input, upper_input, lower_output, upper_output, strides_input, strides_output):
    """See `cufftXtSetDistribution`."""
    cdef nullable_unique_ptr[ vector[int64_t] ] _lower_input_
    get_resource_ptr[int64_t](_lower_input_, lower_input, <int64_t*>NULL)
    cdef nullable_unique_ptr[ vector[int64_t] ] _upper_input_
    get_resource_ptr[int64_t](_upper_input_, upper_input, <int64_t*>NULL)
    cdef nullable_unique_ptr[ vector[int64_t] ] _lower_output_
    get_resource_ptr[int64_t](_lower_output_, lower_output, <int64_t*>NULL)
    cdef nullable_unique_ptr[ vector[int64_t] ] _upper_output_
    get_resource_ptr[int64_t](_upper_output_, upper_output, <int64_t*>NULL)
    cdef nullable_unique_ptr[ vector[int64_t] ] _strides_input_
    get_resource_ptr[int64_t](_strides_input_, strides_input, <int64_t*>NULL)
...
cpdef make_reshape(intptr_t handle, size_t element_size, int rank, lower_input, upper_input, strides_input, lower_output, upper_output, strides_output, intptr_t comm_handle, int comm_type):
    # get_version() call takes about 100-200 ns
    cdef int version = get_version()
    if version < 11400:
        attach_reshape_comm(handle, comm_type, comm_handle)
        make_reshape_11_2(handle, element_size, rank, lower_input, upper_input, lower_output, upper_output, strides_input, strides_output)
    else:
        make_reshape_11_4(handle, element_size, rank, lower_input, upper_input, strides_input, lower_output, upper_output, strides_output, comm_handle, comm_type)


cpdef intptr_t set_descriptor_data(intptr_t descriptor, intptr_t data, int subformat):
    """This is not part of the cuFFTMp API. This is a hack to allow calling
    xt_exec_descriptor by reusing a descriptor. The motivation is that the regular
    xt_exec API allows us to pass our own data buffers and set the direction, but
    doesn't allow setting the subformat (distribution)."""
    cdef cudaLibXtDesc* desc = <cudaLibXtDesc*>descriptor
    cdef intptr_t ptr = <intptr_t>(desc.descriptor.data[0])
    desc.descriptor.data[0] = <void*>data
    desc.subFormat = subformat
    # return the previous data ptr
    return ptr
